...
import { Mail, Phone, BadgePercent, Building, Briefcase } from "lucide-react";
...
export type EditTeacherProfileFormValues = z.infer<typeof editTeacherProfileSchema>;

interface TeacherEditProfileFormProps {
  teacherData: Teacher;
  onSave: (values: EditTeacherProfileFormValues) => Promise<void>;
  onCancel: () => void;
  isSaving: boolean;
}

export default function TeacherEditProfileForm({ teacherData, onSave, onCancel, isSaving }: TeacherEditProfileFormProps) {
  const form = useForm<EditTeacherProfileFormValues>({
  ...
  const departmentLabel = DEPARTMENTS.find(d => d.value === teacherData.department)?.label || teacherData.department || "N/A";
      ...
      <form onSubmit={form.handleSubmit(onSave)} className="space-y-6">
        ...
        <CardContent className="p-0 space-y-6">
          ...
          <div>
            ...
            <Input value={teacherData.email || "N/A"} readOnly className="mt-1 bg-muted/30" />
          </div>
          ...
          <div>
            ...
            <Input value={teacherData.username || 'Not Assigned'} readOnly className="mt-1 bg-muted/30" />
          </div>
          
          <div>
            <FormLabel className="flex items-center text-muted-foreground"><Building className="mr-2 h-4 w-4" />Department (Read-only)</FormLabel>
            <Input value={departmentLabel} readOnly className="mt-1 bg-muted/30" />
          </div>
          <div>
              <FormLabel className="flex items-center text-muted-foreground"><Briefcase className="mr-2 h-4 w-4" />Account Status (Read-only)</FormLabel>
              <Input value={teacherData.status || "N/A"} readOnly className="mt-1 bg-muted/30 capitalize" />
          </div>
